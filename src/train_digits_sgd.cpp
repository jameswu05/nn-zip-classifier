...
int main() {
    ...
    std::ofstream fout("../data/errors_sgd.csv");
    for (size_t i = 0; i < errors.size(); i++) {
        fout << i << "," << errors[i] << "\n";
    }
    fout.close();

    std::cout << "Errors saved to data/errors_sgd.csv." << std::endl;
    ...
    std::cout << "Errors saved to plots/errors_sgd.png." << std::endl;

    plot_decision_boundary_from_NN(
        network,
        X_train,
        "../src/zipDigitsRandom.train",
        "../data/grid_predictions_SGD.csv",
        "../src/plot_decision_boundary.py",
        "../plots/decision_boundary_sgd.png",
        "Stochastic Gradient Descent"
    );
    std::cout << "Decision boundary saved to plots/decision_boundary_sgd.png." << std::endl;
